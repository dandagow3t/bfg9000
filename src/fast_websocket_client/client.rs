use super::{fragment, handshake, tls_connector};
...
impl Offline {
    ...
    pub async fn connect(
        ...
        let (mut ws, _) = match url.scheme() {
            "wss" | "https" => {
                let tls_connector = tls_connector::get_tls_connector();
                ...
                handshake::get_websocket(&SpawnExecutor, request, tls_stream).await?
            }
            _ => handshake::get_websocket(&SpawnExecutor, request, tcp_stream).await?,
        };
        ...
        Ok(Online(fragment::FragmentCollector::new(ws)))
    }
}

/// Provides receive/send functions and configuration setters.
pub struct Online(
    fragment::FragmentCollector<hyper_util::rt::tokio::TokioIo<hyper::upgrade::Upgraded>>,
);
